#include "UltraMark.hpp"

using namespace std;

UltraMark::UltraMark(const std::string& mnn_path,
...
int UltraMark::detect(cv::Mat& raw_image, std::vector<FaceInfo>& face_list) {
    if (raw_image.empty()) {
...
void UltraMark::generateBBox(std::vector<FaceInfo>& bbox_collection, MNN::Tensor* scores, MNN::Tensor* boxes) {
...
void UltraMark::nms(std::vector<FaceInfo>& input, std::vector<FaceInfo>& output, int type) {
